...
-- Usage: Group users by department, region, or function
-- Dependencies: organizations, profiles (manager)
-- ----------------------------------------------------------------------------
CREATE TABLE teams (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Note: Created BEFORE companies to avoid circular dependency
-- 
-- RELATIONSHIPS:
...
CREATE TABLE contacts (
  ...
  -- COMPANY & CLASSIFICATION (company_id will be added after companies table is created)
...
-- Add company_id foreign key to contacts now that companies table exists
...
-- Note: Created AFTER contacts to avoid circular dependency
-- 
-- RELATIONSHIPS:
...
-- ----------------------------------------------------------------------------
-- TABLE: calendar_events (Universal Booking & Calendar System)
-- Purpose: Unified calendar for jobs, personal events, birthdays, holidays, leave
-- Usage: Schedule management, 3rd-party calendar sync (Google, Outlook, iCal)
-- Features: Recurring events, reminders, attendees, timezone support, iCal export
-- ----------------------------------------------------------------------------
CREATE TABLE calendar_events (
  ...
  event_type TEXT NOT NULL CHECK (event_type IN ('job', 'appointment', 'meeting', 'personal', 'birthday', 'holiday', 'leave', 'sick_leave', 'training', 'deadline', 'reminder')),
  category TEXT,                                                              -- Custom category/label for filtering (e.g., 'Client Meeting', 'Team Event', 'Public Holiday')
  
  -- Event Details
  title TEXT NOT NULL,                                                        -- Event title/summary (e.g., 'Install HVAC - 123 Main St', 'Team Standup', 'Christmas Day')
  description TEXT,                                                           -- Detailed description or notes
  location TEXT,                                                              -- Human-readable location (e.g., 'Meeting Room A', '123 Main St, Brisbane')
  
  -- Timing
  start_time TIMESTAMPTZ NOT NULL,                                            -- Event start date/time (timezone-aware)
  end_time TIMESTAMPTZ NOT NULL,                                              -- Event end date/time (timezone-aware)
  timezone TEXT DEFAULT 'UTC',                                                -- Timezone (e.g., 'Australia/Brisbane', 'America/New_York')
  all_day BOOLEAN DEFAULT false,                                              -- All-day event flag (birthdays, holidays)
  
  -- Recurrence (RFC 5545 RRULE for calendar compatibility)
  is_recurring BOOLEAN DEFAULT false,                                         -- Recurring event flag
  recurrence_rule TEXT,                                                       -- iCal RRULE format (e.g., 'FREQ=WEEKLY;BYDAY=MO,WE,FR;UNTIL=20261231T235959Z')
  recurrence_end_date TIMESTAMPTZ,                                            -- Last occurrence date (for UI display)
  parent_event_id UUID REFERENCES calendar_events(id) ON DELETE CASCADE,      -- Link to parent recurring event (NULL for parent, UUID for instances)
  
  -- Relationships (Polymorphic Links)
  linked_entity_id UUID,                                                      -- Polymorphic FK to related entity (job, contact, opportunity, etc.)
  linked_entity_type TEXT CHECK (linked_entity_type IN ('job', 'contact', 'company', 'opportunity', 'quote', 'task', 'profile')),
  job_id UUID REFERENCES jobs(id) ON DELETE CASCADE,                          -- Direct FK to jobs (for job-type events)
  contact_id UUID REFERENCES contacts(id) ON DELETE SET NULL,                 -- Related contact (customer, colleague)
  
  -- Ownership & Participants
  owner_id UUID REFERENCES profiles(id) ON DELETE CASCADE NOT NULL,           -- Event owner/creator (whose calendar this appears on)
  assigned_to UUID REFERENCES profiles(id) ON DELETE SET NULL,                -- Primary assigned person (technician, sales rep, etc.)
  attendees JSONB DEFAULT '[]',                                               -- Array of attendees: [{"profile_id": "...", "email": "...", "name": "...", "status": "accepted|declined|tentative|needs-action"}]
  
  -- Status & Visibility
  status TEXT DEFAULT 'confirmed' CHECK (status IN ('confirmed', 'tentative', 'cancelled', 'completed')),
  visibility TEXT DEFAULT 'default' CHECK (visibility IN ('public', 'private', 'confidential', 'default')),  -- Privacy setting
  busy_status TEXT DEFAULT 'busy' CHECK (busy_status IN ('free', 'busy', 'tentative', 'out_of_office')),    -- Free/busy status for scheduling
  
  -- Reminders & Notifications
  reminders JSONB DEFAULT '[]',                                               -- Array of reminders: [{"minutes_before": 30, "method": "email|sms|push|popup"}]
  
  -- 3rd-Party Calendar Integration
  external_calendar_id TEXT,                                                  -- External calendar ID (Google Calendar ID, Outlook Calendar ID)
  external_event_id TEXT,                                                     -- External event ID from synced calendar (for bidirectional sync)
  ical_uid TEXT UNIQUE,                                                       -- Unique iCal UID (RFC 5545) for calendar export/import
  last_synced_at TIMESTAMPTZ,                                                 -- Last sync timestamp with external calendar
  sync_status TEXT CHECK (sync_status IN ('synced', 'pending', 'failed', 'disabled')),
  
  -- Appearance & Customization
  color TEXT DEFAULT '#3B82F6',                                               -- Event color (hex code) for calendar UI
  ...
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  deleted_at TIMESTAMPTZ                                                      -- Soft delete (cancelled events retain history)
);

-- Field Comments
COMMENT ON COLUMN calendar_events.id IS 'Primary key - unique identifier for the calendar event';
COMMENT ON COLUMN calendar_events.organization_id IS 'Multi-tenant FK - which organization owns this event';
COMMENT ON COLUMN calendar_events.event_type IS 'Event type: job (service appointment) | appointment | meeting | personal | birthday | holiday | leave | sick_leave | training | deadline | reminder';
COMMENT ON COLUMN calendar_events.category IS 'Custom category for filtering and grouping (e.g., "Client Meeting", "Team Event", "Public Holiday", "Annual Leave")';
COMMENT ON COLUMN calendar_events.title IS 'Event title/summary displayed in calendar (e.g., "Install HVAC - 123 Main St", "Team Standup", "John''s Birthday")';
COMMENT ON COLUMN calendar_events.description IS 'Detailed event description, agenda, or notes';
COMMENT ON COLUMN calendar_events.location IS 'Human-readable location (e.g., "Meeting Room A", "123 Main St, Brisbane", "Virtual - Zoom Link")';
COMMENT ON COLUMN calendar_events.start_time IS 'Event start date/time (timezone-aware TIMESTAMPTZ)';
COMMENT ON COLUMN calendar_events.end_time IS 'Event end date/time (timezone-aware TIMESTAMPTZ)';
COMMENT ON COLUMN calendar_events.timezone IS 'Timezone for event display (e.g., "Australia/Brisbane", "America/New_York", "Europe/London")';
COMMENT ON COLUMN calendar_events.all_day IS 'All-day event flag - true for birthdays, holidays, leave (ignores time portion)';
COMMENT ON COLUMN calendar_events.is_recurring IS 'Recurring event flag - true if event repeats';
COMMENT ON COLUMN calendar_events.recurrence_rule IS 'iCal RRULE format (RFC 5545) for recurrence: "FREQ=WEEKLY;BYDAY=MO,WE,FR;UNTIL=20261231T235959Z" - compatible with Google Calendar, Outlook, iCal';
COMMENT ON COLUMN calendar_events.recurrence_end_date IS 'Last occurrence date for recurring events (denormalized for quick queries)';
COMMENT ON COLUMN calendar_events.parent_event_id IS 'FK to parent recurring event - NULL for parent/one-time events, UUID for recurring instances';
COMMENT ON COLUMN calendar_events.linked_entity_id IS 'Polymorphic FK - ID of related entity (job, contact, opportunity, etc.)';
COMMENT ON COLUMN calendar_events.linked_entity_type IS 'Polymorphic type - entity type linked to this event (job | contact | company | opportunity | quote | task | profile)';
COMMENT ON COLUMN calendar_events.job_id IS 'Direct FK to jobs table - for job-type events (quick lookup for field service scheduling)';
COMMENT ON COLUMN calendar_events.contact_id IS 'FK to contacts - related customer or colleague for this event';
COMMENT ON COLUMN calendar_events.owner_id IS 'FK to profiles - event owner (whose calendar this appears on, required)';
COMMENT ON COLUMN calendar_events.assigned_to IS 'FK to profiles - primary assigned person (technician for jobs, sales rep for meetings, etc.)';
COMMENT ON COLUMN calendar_events.attendees IS 'JSONB array of attendees: [{"profile_id": "uuid", "email": "user@example.com", "name": "John Doe", "status": "accepted|declined|tentative|needs-action", "is_organizer": false}]';
COMMENT ON COLUMN calendar_events.status IS 'Event status: confirmed (scheduled) | tentative (maybe) | cancelled (deleted but retained) | completed (finished)';
COMMENT ON COLUMN calendar_events.visibility IS 'Privacy level: public (visible to all) | private (owner only) | confidential (hidden details) | default (org default)';
COMMENT ON COLUMN calendar_events.busy_status IS 'Free/busy indicator for scheduling: free | busy | tentative | out_of_office (for availability checking)';
COMMENT ON COLUMN calendar_events.reminders IS 'JSONB array of reminders: [{"minutes_before": 30, "method": "email"}, {"minutes_before": 10, "method": "push"}] - method: email | sms | push | popup';
COMMENT ON COLUMN calendar_events.external_calendar_id IS 'External calendar ID from synced service (Google Calendar ID, Outlook Calendar ID) for bidirectional sync';
COMMENT ON COLUMN calendar_events.external_event_id IS 'External event ID from synced calendar (Google Event ID, Outlook Event ID) - used for updates/deletes';
COMMENT ON COLUMN calendar_events.ical_uid IS 'Unique iCal UID (RFC 5545) - globally unique identifier for calendar export/import compatibility';
COMMENT ON COLUMN calendar_events.last_synced_at IS 'Last successful sync timestamp with external calendar service';
COMMENT ON COLUMN calendar_events.sync_status IS 'Sync status: synced (up-to-date) | pending (waiting for sync) | failed (sync error) | disabled (sync turned off)';
COMMENT ON COLUMN calendar_events.color IS 'Event color for calendar UI (hex code, e.g., "#3B82F6" for blue, "#EF4444" for red)';
COMMENT ON COLUMN calendar_events.tags IS 'Searchable tags array for filtering and categorization';
COMMENT ON COLUMN calendar_events.created_by IS 'User who created this event';
COMMENT ON COLUMN calendar_events.created_at IS 'Timestamp when event was created';
COMMENT ON COLUMN calendar_events.updated_at IS 'Auto-updated via trigger on every UPDATE';
COMMENT ON COLUMN calendar_events.deleted_at IS 'Soft delete timestamp - NULL means active, timestamp means cancelled (retains history for audit)';

-- Indexes
CREATE INDEX idx_calendar_events_organization ON calendar_events(organization_id) WHERE deleted_at IS NULL;
COMMENT ON INDEX idx_calendar_events_organization IS 'Find all events for an organization';

CREATE INDEX idx_calendar_events_owner ON calendar_events(owner_id) WHERE deleted_at IS NULL;
COMMENT ON INDEX idx_calendar_events_owner IS 'Find events for a specific user (calendar view)';

CREATE INDEX idx_calendar_events_assigned ON calendar_events(assigned_to) WHERE deleted_at IS NULL;
COMMENT ON INDEX idx_calendar_events_assigned IS 'Find events assigned to a technician/user';

CREATE INDEX idx_calendar_events_type ON calendar_events(event_type) WHERE deleted_at IS NULL;
COMMENT ON INDEX idx_calendar_events_type IS 'Filter events by type (job, leave, holiday, etc.)';

CREATE INDEX idx_calendar_events_time_range ON calendar_events(start_time, end_time) WHERE deleted_at IS NULL;
COMMENT ON INDEX idx_calendar_events_time_range IS 'Query events by date range (calendar month/week view)';

CREATE INDEX idx_calendar_events_job ON calendar_events(job_id) WHERE deleted_at IS NULL AND job_id IS NOT NULL;
COMMENT ON INDEX idx_calendar_events_job IS 'Find calendar event linked to a specific job';

CREATE INDEX idx_calendar_events_contact ON calendar_events(contact_id) WHERE deleted_at IS NULL AND contact_id IS NOT NULL;
COMMENT ON INDEX idx_calendar_events_contact IS 'Find events related to a specific contact';

CREATE INDEX idx_calendar_events_recurring ON calendar_events(is_recurring, parent_event_id) WHERE deleted_at IS NULL;
COMMENT ON INDEX idx_calendar_events_recurring IS 'Query recurring events and their instances';

CREATE INDEX idx_calendar_events_external ON calendar_events(external_calendar_id, external_event_id) WHERE deleted_at IS NULL;
COMMENT ON INDEX idx_calendar_events_external IS 'Lookup events by external calendar service IDs (for sync operations)';

CREATE INDEX idx_calendar_events_ical_uid ON calendar_events(ical_uid) WHERE deleted_at IS NULL;
COMMENT ON INDEX idx_calendar_events_ical_uid IS 'Lookup events by iCal UID (for import/export compatibility)';

-- GIN indexes for arrays
CREATE INDEX idx_calendar_events_tags ON calendar_events USING GIN(tags) WHERE deleted_at IS NULL;
COMMENT ON INDEX idx_calendar_events_tags IS 'Search events by tags';

CREATE INDEX idx_calendar_events_attendees ON calendar_events USING GIN(attendees) WHERE deleted_at IS NULL;
COMMENT ON INDEX idx_calendar_events_attendees IS 'Query events by attendees (find all meetings for a user)';

-- Trigger
CREATE TRIGGER set_calendar_events_updated_at
  BEFORE UPDATE ON calendar_events
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
COMMENT ON TRIGGER set_calendar_events_updated_at ON calendar_events IS 'Auto-update updated_at timestamp on every UPDATE';
